...
int nk_bufreader_next(nk_bufreader *r) {
    ...
    log_trace("\x1b[33m1st\x1b[m memchr('\\n', v.len=%d):", VALID_LEN(r));
    debug_print(r);

    // (3.)
    log_trace("Call read(%d) at [%d]", BYTES_TO_READ(r), VALID_LEN(r));
    switch (n = read(r->fd, r->end, BYTES_TO_READ(r))) {
    case 0: // No bytes were read, and end of file is reached.
        if (r->end == r->buf) {
            r->newl = r->end;
            log_trace("\x1b[31mReturn\x1b[m {#2}");
            return NK_BUFREAD_ITER_OVER;
        }
        return NK_BUFREAD_OK;
    ...
    // (4.)
    log_trace("\x1b[34m2nd\x1b[m memchr('\\n', v.len=%d):", VALID_LEN(r));
    r->newl = (char *)memchr(r->buf, '\n', sizeof(char) * VALID_LEN(r));
    debug_print(r);
    if (r->newl) {
        ...
        if (r->newl + 2 == r->buf + r->len) {
            *r->buf = '\0', r->end = NULL, r->newl = r->buf;
            log_trace("\x1b[31mReturn\x1b[m {#3}");
            return NK_BUFREAD_INSUFFICIENT_SPACE;
        }
        *r->newl = '\0';
        log_trace("\x1b[31mReturn\x1b[m {#4}");
        return NK_BUFREAD_OK;
    } else if (VALID_LEN(r) + 1 >= r->len) {
        *r->buf = '\0', r->end = NULL, r->newl = r->buf;
        log_trace("\x1b[31mReturn\x1b[m {#5}");
        return NK_BUFREAD_INSUFFICIENT_SPACE;
    }
    r->newl = r->end;
    debug_print(r);
    log_trace("\x1b[31mReturn\x1b[m {#6}");
    return NK_BUFREAD_OK;
}

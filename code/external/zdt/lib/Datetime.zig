...
// SPDX-FileContributor: Christopher Davis <christopherdavis@gnome.org>
...
pub const Weekday = enum(u8) {
    ...
    pub fn nameToInt(name: []const u8) RangeError!u8 {
        for (0.., std.meta.fieldNames(Weekday)) |i, fname| {
            if (std.mem.eql(u8, name, fname)) return @intCast(i);
        }
    ...
    pub fn nameShortToInt(name: []const u8) RangeError!u8 {
        for (0.., std.meta.fieldNames(Weekday)) |i, fname| {
            if (std.mem.eql(u8, name, fname[0..3])) return @intCast(i);
        }
        return RangeError.DayOutOfRange;
    }
};
...
pub const Month = enum(u8) {
    ...
    pub fn nameToInt(name: []const u8) RangeError!u8 {
        for (1.., std.meta.fieldNames(Month)) |i, fname| {
            if (std.mem.eql(u8, name, fname)) return @intCast(i);
        }
...
/// The current time with nanosecond resolution, based on `std.Io.Clock.real`.
/// If 'null' is supplied as tz_options, naive datetime resembling UTC is returned.
pub fn now(io: std.Io, opts: ?tz_options) ZdtError!Datetime {
    const _now = std.Io.Clock.real.now(io);
...
/// Get current UTC time, based on `std.Io.Clock.real`.
/// Fail-safe since it sets UTC as a pre-defined time zone.
...
/// Get current TAI time / UTC /wo leap seconds.
/// Based on `std.Io.Clock.real`.
pub fn nowTAI(io: std.Io) Datetime {
    const _now = std.Io.Clock.real.now(io);
    const leaps = cal.leapCorrection(@intCast(@divFloor(_now.nanoseconds, ns_per_s)));
    return Datetime.fromUnix(
        _now.nanoseconds + @as(u64, leaps) * ns_per_s,
        Duration.Resolution.nanosecond,
        null,
    ) catch unreachable;
}
...
pub fn weekday(dt: Datetime) Weekday {
    return @as(Weekday, @enumFromInt(dt.weekdayNumber()));
}

pub fn monthEnum(dt: Datetime) Month {
    return @as(Month, @enumFromInt(dt.month));
}

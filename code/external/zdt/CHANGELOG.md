...
- updated benchmark code so it works with zig 0.16+
- examples: use juicy main
- tzdb to 2026c

### Removed

- build.zig: libc linking for zdt module. Since we use external functions from libc, any module *using* zdt must still link libc using
...
- script to check version consistency (LLM-generated)
- Datetime method to get TAI (currently UTC + 37s)
- Timezones: test for Canada / BC on constant DST offset (-7h)

### Changed

- windows_mdnames, unix_mdnames: define external functions locally (system libraries) so that @cImport is not needed anymore
- windows_mdnames, unix_mdnames: define constants (name indices / pointers) locally
...
- updated zdt error set

## 2026-03-03, v0.9.1-zig_master

### Added

- CI script, zig-0.15 branch ([#28](https://codeberg.org/FObersteiner/zdt/pulls/28))
- CI script, main branch (by @pollend, [#31](https://codeberg.org/FObersteiner/zdt/pulls/31))

### Changed

- BREAKING: introduce io interface for `Timezone.fromTzdata` and `Timezone.fromSystemTzdata` (by @pollend, [#23](https://codeberg.org/FObersteiner/zdt/pulls/23), [#25](https://codeberg.org/FObersteiner/zdt/pulls/25))
- BREAKING: Datetime.now and Datetime.UTCnow API now needs `io` to be passed since we use std.Io.Clock ([#29](https://codeberg.org/FObersteiner/zdt/pulls/29))
- use juicy main in demo
- tzdata version to 2026a

## 2025-12-11, v0.9.0-zig_master

This release targets the latest Zig master.

### Changed

- bump IANA timezone database to 2025c release

### Fixed

- Datetime, getSurroundingTimetypes function: fix type casting of transitions index to usize
